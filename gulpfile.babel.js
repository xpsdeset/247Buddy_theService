// Generated on 2016-09-18 using generator-angular-fullstack 3.8.0
'use strict';
...
gulp.task('start:server:prod', () => {
    ...
    nodemon(`-L-w ${paths.dist}/${serverPath} ${paths.dist}/${serverPath}`)
        .on('log', onServerLog);
});
...
gulp.task('watch', () => {
    ...
    gulp.watch(paths.client.styles, () => {  //['inject:scss']
    ...
    plugins.watch('.tmp/app/*.css', function(file){
        plugins.livereload.changed(file)
    });
    gulp.watch(paths.client.scripts,['transpile:client']) 
    gulp.watch(paths.server.scripts,['transpile:server'])
...
gulp.task('serve:debug', cb => {
    runSequence(['clean:tmp', 'constant'],
        ...
        ['transpile:client', 'styles'],
        'start:inspector',
        ['mongo-start','start:server:debug', 'start:client'],
        'watch',
        cb);
});
...
gulp.task('copy:extras', () => {
    return gulp.src([
        ...
        `${clientPath}/.htaccess`,
        `${clientPath}/google16e4b654a1bfccd6.html`,
        `${clientPath}/*.png`
    ], { dot: true })
        .pipe(gulp.dest(`${paths.dist}/${clientPath}`));
});
...
grunt.initConfig({
    buildcontrol: {
        ...
        heroku: {
            options: {
                remote: 'https://git.heroku.com/demo247buddy.git',
                branch: 'master'
            }
        },
        openshift: {
...
gulp.task('heroku',['build'], function(done) {
    grunt.tasks(
